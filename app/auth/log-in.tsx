      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <Button label="New here? Create an account" variant="ghost" onPress={() => router.push('/auth/sign-up')} style={{ marginTop: Spacing.xs }} />
          </View>
          <View style={styles.devRow}>
            <Button label="🎮 Dev Preview — Skip Login" variant="secondary" onPress={handleDevPreview} style={{ borderColor: Colors.gold }} />
...
const styles = StyleSheet.create({
  ...
  logoBlock: { alignItems: 'center', gap: 4, marginBottom: Spacing.sm },
  logoTop: { fontSize: 11, fontWeight: '700', letterSpacing: 4, color: Colors.gold, textTransform: 'uppercase' },
  logoMain: { fontSize: 36, fontWeight: '900', letterSpacing: 3 },
  logoSub: { fontSize: 12, color: Colors.textMuted, letterSpacing: 2, textTransform: 'uppercase' },
  card: { backgroundColor: 'rgba(7,11,20,0.85)', borderRadius: 16, borderWidth: 1, borderColor: 'rgba(0,200,255,0.25)', padding: Spacing.lg },
  devRow: { gap: Spacing.xs, paddingBottom: Spacing.lg },
  devLabel: { textAlign: 'center', fontSize: 10, color: Colors.textDim, letterSpacing: 0.5 },
});
